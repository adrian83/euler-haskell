...
palindrome (x:xs) = let
    l = last xs
  in
    if x == l then palindrome $ init xs else False
...
isBla numb acc numbs org =
    if numb > acc then  isBla numb (acc + (head numbs)) (tail numbs) org else (
      if numb == acc then True else isBla numb (acc - (head org)) numbs (tail org)
    )
